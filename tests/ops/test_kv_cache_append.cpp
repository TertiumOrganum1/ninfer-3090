...
// Independent CPU oracle for the rk4v4-e8 key coding. The row is rotated with the same butterfly
// order as the device, each G64 group gets an FP16-RNE(absmax/7) scale, and every block of eight
// scaled dimensions snaps to the nearest E8 point before the RNE code is clamped to [-8, 7].
struct E8OracleStats {
    int coset_blocks  = 0;
    int parity_fixes  = 0;
    int clamped_codes = 0;
};
...
E8Block nearest_d8_host(const E8Block& y, int& parity_fixes) {
    E8Block r{};
    int odd        = 0;
    int worst_lane = 0;
    float worst    = -1.0f;
    for (int i = 0; i < 8; ++i) {
        r[static_cast<std::size_t>(i)] =
            static_cast<float>(round_even_to_i32(y[static_cast<std::size_t>(i)]));
        odd ^= static_cast<int>(r[static_cast<std::size_t>(i)]) & 1;
        const float error = std::abs(y[static_cast<std::size_t>(i)] - r[static_cast<std::size_t>(i)]);
        if (error > worst) {
            worst      = error;
            worst_lane = i;
        }
    ...
    if (odd != 0) {
        auto& fixed = r[static_cast<std::size_t>(worst_lane)];
...
// Butterfly order of the device's octet reduction: ((0+1)+(2+3))+((4+5)+(6+7)).
float e8_sq_distance_host(const E8Block& x, const E8Block& p) {
    std::array<float, 8> sq{};
    for (int i = 0; i < 8; ++i) {
        const float d = x[static_cast<std::size_t>(i)] - p[static_cast<std::size_t>(i)];
        sq[static_cast<std::size_t>(i)] = d * d;
    }
    return ((sq[0] + sq[1]) + (sq[2] + sq[3])) + ((sq[4] + sq[5]) + (sq[6] + sq[7]));
}

void encode_full_row_e8_keys(std::array<float, kFullHeadDim> row, std::vector<std::uint8_t>& packed,
    ...
    for (int group = 0; group < kFullGroups; ++group) {
        ...
        for (int block = 0; block < kFullGroup / 8; ++block) {
            ...
            const bool coset = e8_sq_distance_host(y, coset_point) < e8_sq_distance_host(y, integer_point);
            ...
            for (int i = 0; i < 8; ++i) {
                const int raw  = round_even_to_i32(point[static_cast<std::size_t>(i)]);
                const int code = std::clamp(raw, -8, 7);
                stats.clamped_codes += raw != code ? 1 : 0;
                const int d       = d0 + i;
                const auto target = full_cache_index(kFullHeadDim / 2, d / 2, head, position,
                                                     physical_page, kv_heads);
                const auto nibble = static_cast<std::uint8_t>(static_cast<unsigned>(code) & 0x0fu);
                if ((d & 1) == 0) {
                    packed[target] = static_cast<std::uint8_t>((packed[target] & 0xf0u) | nibble);
                    ...
                    packed[target] =
                        static_cast<std::uint8_t>((packed[target] & 0x0fu) | (nibble << 4));
                }
            }
        }
    }
...
// rk4v4-e8: both code planes are half-width packed int4. Unlike rk8v4's INT8 keys, the key plane
// is checked bit for bit against the E8 oracle; values reuse the rk8v4 value oracle.
int full_append_case_e8(int kv_heads, int tokens = 3) {
    const int first_position = tokens >= 128 ? 61 : 63;
    const int logical_pages  = (first_position + tokens + kPage - 1) / kPage;
    const int physical_pages = 2 * logical_pages + 1;
    std::vector<std::int32_t> mapping(static_cast<std::size_t>(logical_pages));
    ...
    std::vector<float> host_k(elements);
    std::vector<float> host_v(elements);
    fill_uniform(host_k, 0x61u, -6.0f, 6.0f);
    fill_uniform(host_v, 0x62u, -6.0f, 6.0f);
    round_to_bf16(host_k);
    round_to_bf16(host_v);
    std::vector<std::uint16_t> input_k(elements);
    std::vector<std::uint16_t> input_v(elements);
    ...
    GuardedDeviceBuffer cache_k(packed_count);
    GuardedDeviceBuffer cache_v(packed_count);
    GuardedDeviceBuffer scale_k(scale_count * sizeof(std::uint16_t));
    GuardedDeviceBuffer scale_v(value_scale_count * sizeof(std::uint16_t));

    std::vector<std::uint8_t> expected_k(packed_count, 0x5aU);
    std::vector<std::uint8_t> expected_v(packed_count, 0xa5U);
    auto expected_scale_k = patterned_bits(scale_count, 0x01234567u);
    auto expected_scale_v = patterned_bits(value_scale_count, 0x89abcdefu);
    ...
    for (int token = 0; token < tokens; ++token) {
        ...
        for (int head = 0; head < kv_heads; ++head) {
            ...
            encode_full_row_e8_keys(row, expected_k, head, position, page, kv_heads,
    ...
    const std::string label = "kv_cache_append full rk4v4-e8 Hkv=" + std::to_string(kv_heads) +
                              " T=" + std::to_string(tokens) +
                              " P=" + std::to_string(first_position);
    ...
    failures += verify_exact((label + " k codes").c_str(),
                             from_device<std::uint8_t>(cache_k.data(), packed_count), expected_k);
...
int main(int argc, char** argv) {
    ...
    for (const int kv_heads : {2, 4}) {
        ...
        failures += full_append_case_e8(kv_heads);
    }
