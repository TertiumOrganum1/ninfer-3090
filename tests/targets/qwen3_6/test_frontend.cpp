...
// Not named `near`: minwindef.h defines near/far as empty legacy
// memory-model macros, which mangles the declaration on MSVC.
...
// Same merge contract as test_tokenizer_config_merge(), but against the official Qwen3.6-27B HF
// export's real tokenizer_config.json rather than the in-repo fixture. Only runs when that
// non-redistributable export is available locally.
...
int test_forced_tool_call(const Frontend& frontend) {
    ...
    int failures = check(prompt_text.ends_with("<|im_start|>assistant\n<think>\n\n</think>\n\n"
                                               "<tool_call>\n<function=TaskUpdate>\n"),
