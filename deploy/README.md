...
the process itself needs: about 16 GiB of RSS. The previous line capped the placement planner's
search with `--materialization-search-ms 100`, because a fixed 5 ms budget exhausted on 100k-token
multimodal agent prompts and dropped the previous turn's checkpoint. This line carries the upstream
search rework instead: the planner starts from 5 ms and renews its grant while the predicted gain
still pays for the search, bounded by the planning allowance, so the flag no longer exists.
