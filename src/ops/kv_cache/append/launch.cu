...
void launch_full(const Tensor& k, const Tensor& v, const Tensor& positions, CacheView cache,
    ...
    if (cache.storage == KvCacheStorage::Int8Group64 ||
        ...
        Tensor& cache_k_scale = cache.k_scale_pages;
        Tensor& cache_v_scale = cache.v_scale_pages;
        // A U8 value plane is the rk8v4 packed signed int4 coding; I8 is the plain INT8 one.
        // rk4v4-e8 packs the key plane as well. Both kernels take the key plane as std::int8_t*
        // and re-cast it to bytes on the packed path.
        ...
        } else if (cache.v_pages.dtype == DType::U8) {
