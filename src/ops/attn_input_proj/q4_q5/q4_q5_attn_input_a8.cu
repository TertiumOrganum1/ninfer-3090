...
void launch_range(const Weight& weight, const std::int8_t* codes, const __half* scales,
    ...
    a8::a8_mma_kernel<Codec, kHidden, a8::StoreEpilogue>
        <<<grid, a8::kThreads, a8::shared_bytes(), stream>>>(
            ...
            static_cast<const __half*>(weight.scales), codes, scales, tokens, row_begin,
...
void q4_q5_attn_input_a8_launch(const Tensor& x, const Weight& query_key, const Weight& gate_value,
    ...
    launch_range<a8::Q4Codec>(query_key, x_codes, x_scales, tokens, 0, kQRows,
                              static_cast<__nv_bfloat16*>(q.data), kQRows, stream);
    launch_range<a8::Q4Codec>(query_key, x_codes, x_scales, tokens, kQRows, kKvRows,
                              static_cast<__nv_bfloat16*>(k.data), kKvRows, stream);
    launch_range<a8::Q5Codec>(gate_value, x_codes, x_scales, tokens, 0, kQRows,
                              static_cast<__nv_bfloat16*>(gate.data), kQRows, stream);
    launch_range<a8::Q5Codec>(gate_value, x_codes, x_scales, tokens, kQRows, kKvRows,
