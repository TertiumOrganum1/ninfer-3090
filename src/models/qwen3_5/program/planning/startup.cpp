...
void validate_target_options(const execution::Parameters& parameters, DeviceContext& device,
    ...
    // 12.0 is admitted alongside the two qualified capabilities: consumer Blackwell runs the same
    ...
    if (device.compute_capability() != 86 && device.compute_capability() != 89 &&
        device.compute_capability() != 120) {
        throw std::invalid_argument(
            "Qwen3.5 family runtime requires compute capability 8.6, 8.9 or 12.0");
    }
}
