...
private:
    ...
    [[nodiscard]] static bool construction_option_better(const GuidanceCost& parent,
        ...
        if (a && b) {
            ...
            const auto left  = core::wide_multiply(delta(cost), b);
            const auto right = core::wide_multiply(delta(prior), a);
            if (core::wide_less(left, right) || core::wide_less(right, left)) {
                return core::wide_less(left, right);
            }
        }
